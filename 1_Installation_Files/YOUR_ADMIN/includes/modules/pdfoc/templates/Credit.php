...
if ($pageloop == "0") {   // initialize pdf settings
  ...
  $pdf->ezText(PDFOC_ENTRY_CREDIT,PDFOC_GIANT_FONT_SIZE, array('aleft'=>$cx));
  ...
  //
  // This section creates a new credit invoice id (if no credit invoice has
  // previously been generated for this order) OR retrieves the existing id.
  // The credit invoice credits the FULL amount of the original invoice.
  //
  // Note: This implies that there can be only one credit invoice per order id. The
  // assumption here is that if something changes about an order (the customer
  // adds an item after you've sent the invoice, for example), then the order
  // is cancelled, a refund is issued on paper (via this file),
  // and a new order is created with the updated order data. Creation of the
  // new order is outside the scope of OTFIN; the Master Password contribution
  // may be of assistance to you there.

  // search the db to see if this order already has a debit invoice (allowing
  // a credit invoice to be created) and to see if it already has a credit
  // invoice
  //
  ...
  if (!$verify_debit->EOF) { // debit invoice exists, so credit may be created

    if ($verify_credit->EOF) { // no credit invoice exists for this order, so create one now
        ...
        $ot[0] = -1.00*$otx->fields['value'];    // credited tax
        $ot[1] = -1.00*$ott->fields['value'];    // credited total

      // record this invoice in invoices table, then read back out to get newly created invoice id
      $db->Execute("insert into " . TABLE_ORDERS_INVOICES . " (orders_id, invoice_date, invoice_type, order_tax, order_total) values ('". $orders->fields['orders_id'] . "', now(), 'CR', '" . $ot[0] . "', '" . $ot[1] . "')");
      $verify_credit = $db->Execute("select * from " . TABLE_ORDERS_INVOICES . " where invoice_type = 'CR' AND orders_id = '" . $orders->fields['orders_id'] . "'");

    }  // EOIF $verify_credit->EOF

    // one way or another, we now have the credit invoice id for this order,
    // but we'll do some error-checking just in case
    //
    if (!$verify_credit->EOF) {  // all is as it should be

      $invoice_id = $verify_credit->fields['orders_invoices_id'];
      $invoice_date = zen_date_long($verify_credit->fields['invoice_date']);
      ...
      $message .= '<br />' . PDFOC_MESSAGE_ERROR_OCCURRED;

    }  // EOIF !$verify_credit->EOF

  } else { // this order doesn't have a debit invoice, so you can't create a credit number

    $invoice_id = 'n/a';
    $invoice_date = 'n/a';

    $message .= '<br />' . sprintf(PDFOC_MESSAGE_NO_DEBIT_EXISTS, $orders->fields['orders_id']);
...
//  $pdf->ezSetY($pos);
  ...
  // ----- BOF OTFIN -----
  //
  // Print out the prices in the products table with "-" added
  //
    $pdf->ezSetY($pos+PDFOC_PRODUCT_TABLE_ROW_HEIGHT-PDFOC_PRODUCT_TABLE_BOTTOM_MARGIN);
    $x += PDFOC_MODEL_COLUMN_SIZE+ PDFOC_PRICING_COLUMN_SIZES;
    ...
    if ( ($k = sizeof($order->products[$i]['attributes'])) > 0) {
      ...
      for ($j = 0; $j < $k; $j++) {
        ...
        if ($order->products[$i]['attributes'][$j]['price'] != '0') {
           // ----- BOF OTFIN -----
           //
           // Print out the attribute prices in the products table with "-" added
           //
